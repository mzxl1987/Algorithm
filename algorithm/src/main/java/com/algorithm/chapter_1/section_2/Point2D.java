...
public final class Point2D implements Comparable<Point2D> {
    ...
    /**
     * 1.2.1.从命令行接收一个整数N.在正方形中生成N个随机的点,然后计算2点间的距离
     */
    private static void testCase_2(){
        ...
        for (int i = 0; i < N; i++) {
         ...
         // 计算最近的距离
            ...
            double tmpDis;
	        while(j < i){
	        	if((tmpDis = points[i].distanceTo(points[j])) < minDistance){
	        		minIndex = j;
	        		minDistance = tmpDis;
	        	}
	        	j++;
	        ...
	        // 打印最近的距离 & 连线
	        System.out.println("" + i + " : " + points[minIndex] + "," + points[i] + " distance " + points[minIndex].distanceTo(points[i]));
	    	points[minIndex].drawTo(points[i]);
	    	StdDraw.show();
        }
    }
